...
public class Main {
/*Desarrolla un programa en Java que permita gestionar tareas pendientes. El programa debe permitir agregar tareas, marcar tareas como completadas y mostrar
 *  todas las tareas indicando si están completadas o no. Deberemos poder también eliminar todas las tareas completadas o modificar el nombre de una 
 * tarea. Utiliza una ArrayList para almacenar las tareas. Las tareas tienen por lo menos un nombre y un estado por lo que deberíamos crear la clase tareas. */
	...
	//String nuevoNombre = sc.nextLine();
		...
		/*for (int i = 0; i < listaTareas.size(); i++) {
			if(listaTareas.get(i).getNombre().equalsIgnoreCase(nombre)) {
				listaTareas.get(i).setNombre(nuevoNombre);
				i=listaTareas.size();
			}
		} */
	}
